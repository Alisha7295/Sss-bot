...
exec("rm -rf modules/commands/cache && mkdir -p modules/commands/cache && rm -rf modules/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#00ffff")("expert (Shankar) ❯ ") + chalk.hex("#00ffff")("fixed by expert (Shankar)"))
});
...
console.log(chalk.bold.hex("#00ffff").bold("[ Shankar (Shankar) ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger('Hey, thank you for using this Bot', '[ Shankar (hndsm) ]');
        logger("Fixed by Shankar Suman", '[ Shankar (hndsm) ]');
...
cron.schedule('0 2 5 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodmorning everyone, have a nice day (Shankar)", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Kolkata"
