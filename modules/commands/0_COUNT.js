...
var userget = await Users.getAll(['userID'])
    ...
    if (input=="member"){out(`इस ग्रुप में टोटल [${threadInfo.participantIDs.length}] ठरकी है।😒😝🤣👈`)}
    if (input=="male"){out(`इस ग्रुप में टोटल [${gendernam.length}] मम्मी के मगरमच्छ है।🤣🤭👈`)}
    if (input=="female"){out(`इस ग्रुप में टोटल [${gendernu.length}] पापा की परी है🤣🤭👈`)}
    if (input=="gei"){out(`इस ग्रुप में ${nope.length} मेंबर्स है।🤠`)}
